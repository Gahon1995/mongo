...
def reset_db():
    ...
    for dbms in DBMS.all:
        dbs[dbms].drop_database(DBMS.db_name)
...
def gen_data():
    # 71.67s
    logger.info('生成user数据...')
    gen_users()

    logger.info('生成article数据...')
    gen_articles()

    logger.info('生成read数据...')
    gen_reads()

    logger.info('导入populars数据...')
    gen_populars()
...
def main():
    ...
    Config.log_in_file = True
    Config.log_file_name = './genDatabase.log'
    Config.is_import = True
